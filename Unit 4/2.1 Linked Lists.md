...
Addresses are typically represented with an ampersand. When you access an array item by its index, its address is retrieved using the calculation `8*index + &firstItem`, where `&` means "address of". If `&array[0]` is `[I@2a139a55`, then `&array[1]` is `[I@2a139a63`, and the `&array[2]` is `[I@2a139a71`, etc. 
