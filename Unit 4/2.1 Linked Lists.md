...
When you access an array item by its index, its address is retrieved using the calculation `8*index + &firstItem`, where `&` means "address of". If `&array[0]` is `[I@2a139a55`, then `&array[1]` is `[I@2a139a63`, and `&array[2]` is `[I@2a139a71`, etc. 
...
Linked lists are great to use when you want to represent queues, since popping/dequeueing and pushing/enqueueing are both *O(1)*. However, linked lists should not be used whenever you intend to sort the items.
