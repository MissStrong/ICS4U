# [Link to video.](https://www.youtube.com/watch?v=kaAMZm7VN14&list=PLVD25niNi0Bkf1S2xcJ3NaMINjkkfTXxF)
...
A `FileNotFoundException` is thrown by the `Scanner()` function when the argument is not a valid `File` object. That's why we can use `catch (IOException exception)` to handle the situation when we enter a bad filename.
...
public class Main {
     ...
     *
     * @param word
     * @throws IllegalArgumentException when the argument contains any non-alphabet characters
     */
