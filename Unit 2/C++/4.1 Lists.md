...
Another option is to use `next()` (go forward) or `prev()` (go backward), which work like `advance()` except they return the pointer whereas `advance()` is a void function.
