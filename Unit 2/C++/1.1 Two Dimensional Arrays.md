...
The `auto` keyword is used because we can't otherwise use a for-each loop in a 2D array (`int row[] : matrix` doesn't work). `auto` is preceded by the address (`&`) of the variable we are declaring.
